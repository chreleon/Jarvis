...
TUTORIALS: dict[str, dict] = {
    ...
    "secretary": {
        "what": "Secretary mode: Jeeves holds conversations for you and escalates urgent ones.",
        "when": "'secretary on' when you're busy; 'secretary off' to stop. The first time it opens a WhatsApp window with a QR — scan it once with your phone and it stays connected forever. 'link whatsapp' reopens that window anytime. Feed messages from other apps in plain words: \"mom says: dinner at 7?\", \"message from mom: ...\", or \"handle from mom: ...\". 'any messages for me' shows escalated items; \"reply to mom: yes\" answers personally.",
        "example": "link whatsapp",
        "note": "Routine messages get an automatic reply — drafted by Meta AI (WhatsApp's own AI) in your casual style (sheng/lol/ty/emoji) when available, with the instant built-in template as fallback for every failure; the no-commit rule is enforced in the prompt AND by a rejection filter (disable with secretary_meta_ai_drafts: false). Media: photos/videos/documents/GIFs are FORWARDED to Meta AI for real analysis — the safe type-ack goes out instantly, then Meta AI's actual reply about the content (it genuinely reads the photo/doc — a forwarded CV got \"Nimeipata CV yako 💪\") arrives as a follow-up. Voice notes/stickers/locations can't be forwarded, so they keep the type template (wow stunning photo 😍 / professional ack); reactions/recalls are silently skipped with no reply. Replies use what each contact calls YOU — a one-time scan (auto, at most daily; 'secretary scan' re-runs it) learns that the wife's chat says 'Ziii', so replies say \"I'll make sure Ziii sees it\" — a static map, never re-scanned per draft; unknown senders get 'My boss'. Money/urgency/decisions are escalated to you. All contacts are handled; group chats are never replied to. Calls are watched too: incoming audio/video calls (and missed calls in the chat list) are escalated to you, never auto-replied. 'secretary off' ends with a session report — who it talked to, what it told them, calls seen, and anything waiting for your answer. Monitoring scans WhatsApp's Unread section only (small pane, recent-24h first). Set secretary_self_chat (e.g. 'Omoke Jr') in config/api_keys.json to make your own self-chat a remote dashboard — it's ALWAYS on (even with secretary mode off): while the daemon runs, messages to that chat route through the full Jeeves CLI brain and replies come back into the chat; secretary off only stops triaging other people's chats. The monitor also keeps the daemon alive (no idle shutdown). Send a document/photo/video to that chat and Jeeves downloads it (whatsapp_media/) and attaches it like /attach — your next message (e.g. 'summarize') is run against it; 'detach' releases it. Voice notes/stickers aren't downloadable yet. Connects either to your own Chrome (secretary_cdp_url + --remote-debugging-port) or the dedicated WhatsApp window (.whatsapp_profile) — the session is saved, so toggling secretary on/off reuses the same window and you never re-scan. The full tool form is: tool secretary action='handle' sender='Mom' message='dinner at 7?'",
    },
    "agent_task": {
